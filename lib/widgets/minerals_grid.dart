...
/// Minerals Grid Widget
///
/// Displays minerals in a grid format with:
/// - Icon and name
/// - Amount per serving
/// - Percentage of RDA
/// - Color-coded progress bar
class MineralsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Map minerals to their values from nutrition facts
    final mineralData = _getMineralData();
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: AppColorsV2.mintCream.withOpacity(0.3),
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text(
                  '${mineralData.where((m) => m['percentage'] > 10).length}/${mineralData.length}',
          ...
          Padding(
            ...
            child: GridView.builder(
              ...
              itemCount: mineralData.length,
              itemBuilder: (context, index) {
                final data = mineralData[index];
                return _buildMineralCard(
  ...
  List<Map<String, dynamic>> _getMineralData() {
    final mineralMap = {
      'Ca': nutrition.calcium,
      'Fe': nutrition.iron,
      'Mg': nutrition.magnesium,
      'P': nutrition.phosphorus,
      'K': nutrition.potassium,
      'Zn': nutrition.zinc,
    };
    ...
    for (var mineral in minerals) {
      final amount = mineralMap[mineral.code] ?? 0.0;
      final rda = mineral.getRDAForProfile(
        gender: gender,
...
/// Compact Minerals Summary
class CompactMineralsSummary extends StatelessWidget {
  final NutritionFacts nutrition;
  final List<Mineral> minerals;

  const CompactMineralsSummary({
    super.key,
    required this.nutrition,
    required this.minerals,
  });
  ...
  Widget build(BuildContext context) {
    // Get top 3 minerals
    final mineralData = _getTopMinerals(3);
  ...
  List<Map<String, dynamic>> _getTopMinerals(int count) {
    final mineralMap = {
      'Ca': nutrition.calcium,
      'Fe': nutrition.iron,
      'Mg': nutrition.magnesium,
    ...
    for (var mineral in minerals) {
      final amount = mineralMap[mineral.code] ?? 0.0;
      if (amount > 0) {
        final rda = mineral.rdaFemale; // Use default female RDA
        final percentage = rda > 0 ? (amount / rda) * 100 : 0.0;
